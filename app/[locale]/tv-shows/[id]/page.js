...
export default async function TvShowDetailPage({ params }) {
  ...
  try {
    ...
    const seasonsWithEpisodes = await Promise.all(
      (details.seasons ?? []).map(async (season) => {
        try {
          const seasonDetails = await api.getTVSeasonDetails(id, season.season_number);
          return { ...season, episodes: seasonDetails.episodes };
        } catch (e) {
          console.warn(`Failed to fetch season ${season.season_number}:`, e.message);
          return { ...season, episodes: [] };
        }
      })
    );

    tvShow = { ...details, seasons: seasonsWithEpisodes, trailerUrls: details.trailerUrls ?? [] };
  ...
  if (!tvShow) return <DialogMessage message={messages.tvShowLoadError} />;

  const title = tvShow.title?.trim() || fallbacks.notAvailable;
  const backdrop = tvShow.imageUrl || '/not-available.png';
  const posterUrl = tvShow.posterUrl || tvShow.imageUrl || '/not-available.png';
  const genres = deduplicateById(tvShow.genres ?? []);
  const productionCompanies = deduplicateById(tvShow.productionCompanies ?? []);
  const castMembers = deduplicateById(tvShow.cast ?? []);
  const crew = deduplicateById(tvShow.crew ?? []);
  const formattedReleaseDate = tvShow.releaseDate?.trim()
    ? formatDate(tvShow.releaseDate, locale)
    : '';
  ...
  const seasonTabs = (tvShow.seasons ?? []).map((season) => ({
