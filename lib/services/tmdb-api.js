...
export function createTmdbApi(fetchFn, apiKey, language = DEFAULT_LOCALE) {
   ...
   * from the language code, e.g. `de-DE` → `DE`.
   *
    ...
    try {
      ...
      if (regionData) {
        ...
        providers = mappedProviders.length
          ? { link: regionData.link ?? '', providers: mappedProviders }
          : null;
      }
    } catch (error) {
   ...
   * `content_ratings`. The desired region is derived from the language
   * code, e.g. `de-DE` → `DE`.
   *
   * @param {string} mediaType - `movie` or `tv`.
   * @param {number|string} id - TMDB ID.
  ...
  async function getMovieDetails(id) {
    ...
    return { ...mapped, certification, providers };
  }
   ...
   * @param {number} [page=1] - Page number.
   * @returns {Promise<{page:number,results:Object[],hasMore:boolean}>} Normalized search response.
   */
