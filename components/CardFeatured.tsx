...
export default function CardFeatured({
  ...
  const normalizedType: MediaType | null =
  ...
  const featuredType =
    normalizedType === 'movie'
      ...
      : normalizedType === 'tv'
        ? labels.tvShow
        : notAvailableText;
  const featuredImageUrl = imageUrl || notAvailable.src;
  const featuredPosterUrl = posterUrl || imageUrl || notAvailable.src;
      ...
      <div className="featured-card-overlay">
        <div className="featured-card-layout">
          ...
          <div className="featured-card-content">
            <p
              className={`featured-card-type featured-card-type--${
                normalizedType ?? 'unknown'
              }${featuredType ? '' : ' u-not-available'}`}
            >
              {featuredType}
            </p>
            ...
            <p
              className={`featured-card-description${featuredOverview ? '' : ' u-not-available'}`}
            >
              {featuredOverview}
            </p>

            <nav
              aria-label={`Aktionen für ${featuredTitle}`}
              className="featured-card-actions"
            >
              {detailsHref && (
                <Link
