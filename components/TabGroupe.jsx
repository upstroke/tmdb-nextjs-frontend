...
export default function TabGroupe({ tabs = [], initialTab, ariaLabel = '', onTabSelect }) {
  ...
  return (
    <>
      <div className="ui pointing secondary menu tab-bar" role="tablist" aria-label={ariaLabel}>
        {tabs.map((tab, index) => (
      ...
      {tabs.map((tab, tabIndex) => (
        <div key={tab.id} className={`ui tab segment tab-panel${isSelected(tab.id) ? ' active' : ''}`} id={`panel-${tab.id}`} data-tab={tab.id} role="tabpanel" aria-labelledby={`tab-${tab.id}`} hidden={!isSelected(tab.id)}>
          {tab.loading ? (
            <p aria-live="polite">{messages.loading}</p>
          ) : tab.episodes && tab.episodes.length > 0 ? (
            <ol className="episodes-list" ref={(el) => { episodeListRefs.current[tabIndex] = el; }}>
              {tab.episodes.map((episode, episodeIndex) => (
                <li key={episode.id ?? episodeIndex} className="episode-item" tabIndex={isSelected(tab.id) && episodeIndex === focusedEpisodeIndex[tabIndex] ? 0 : -1} onKeyDown={(e) => handleEpisodeKeydown(e, tabIndex, episodeIndex, tab.episodes.length)}>
                  <h4 className="episode-title">{episodeIndex + 1}. {episode.name}</h4>
                  {episode.air_date && (<><span className="u-sr-only">{labels.firstAirDate}</span><time className="episode-air-date">{formatDate(episode.air_date, locale)}</time></>)}
