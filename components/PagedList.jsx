...
export default function PagedList({ initialData, apiPath, storageKey, cardIdPrefix, listKeyPrefix, heading, emptyMessageKey = 'noContent' }) {
  ...
  useEffect(() => {
    ...
    restorePagedList({
      ...
      fetchPageData: async (pageNumber) => {
        const res = await fetch(`/api/${locale}/${apiPath}?page=${pageNumber}`, { headers: { accept: 'application/json' } });
  ...
  async function loadMore() {
    ...
    try {
      ...
      const res = await fetch(`/api/${locale}/${apiPath}?page=${nextPage}`, { headers: { accept: 'application/json' }, signal: controller.signal });
